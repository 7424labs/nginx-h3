...
# Build configuration - limit CPU cores for compilation (adjust as needed)
# Set to 2 to use 2 cores, or use $(nproc) for all available cores
ENV BUILD_CORES=2
...
# Download and build OpenSSL 3.5.4 (with QUIC support)
# Note: OpenSSL 3.x has QUIC support built-in, no special flags needed
RUN curl -L https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz | tar xz && \
    cd openssl-${OPENSSL_VERSION} && \
    ./config --prefix=/build/openssl \
        --openssldir=/etc/ssl \
        --libdir=lib \
        enable-ktls \
        no-tests && \
...
RUN curl -L https://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz | tar xz && \
    ...
    --with-cc-opt="-I/build/openssl/include -O3 -fstack-protector-strong -Wno-error" \
    --with-ld-opt="-L/build/openssl/lib -Wl,-rpath,/usr/local/lib" \
...
COPY --from=builder /build/openssl/lib /usr/local/lib
